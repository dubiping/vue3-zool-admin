...
<script lang="ts" setup>
  ...
  const isShow = (item) => {
    return isFunction(item.show) ? item.show(data) : item.show === false ? false : true;
  };
  const disabled = (item) => {
    return isFunction(item.disabled) ? item.disabled(data) : item.disabled;
  };
  ...
  // 初始化按钮列表
  const initActions = () => {
    const list = actions.filter((v) => isShow(v));

    // 没有设置限制值，或者按钮数量 <= 限制值，直接全部显示，不做更多操作
    if (limit < 1 || list.length <= limit) return [list, []];
